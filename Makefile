...
tag: SQLITE_VER=$(shell cat Dockerfile | grep "ENV SQLITE_VER" | sed -e 's,ENV SQLITE_VER=\(.*\),\1,' | tr -d '\n')
tag: VERSION=musl_$(MUSL_VER)_zlib_$(ZLIB_VER)_ssl_$(SSL_VER)_curl_$(CURL_VER)_sqlite_$(SQLITE_VER)
